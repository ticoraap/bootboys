...
export default class DockList extends Component {
    ...
    render() {
        ...
        return(
            this.filteredDocks.map((value,index) =>{
                return(
                    <div className={'rentableDockItem'} key={index} onClick={() => this.onClickDockHandler(value.dockid)}>
                        <div id={'test'}>
    ...
    filterContainsDockName() {
        this.filteredDocks = this.allDocks.filter(dock => this.compare(dock.name, this.filters[0]))
    }

    filterMaximumPrice() {
        this.filteredDocks = this.filteredDocks.filter(dock => dock.price <= this.filters[2])
    }

    filterMinimumLength() {
        this.filteredDocks = this.filteredDocks.filter(dock => dock.length >= this.filters[1])
    }

    filterMinimumWidth() {
        this.filteredDocks = this.filteredDocks.filter(dock => dock.width >= this.filters[3])
    }
