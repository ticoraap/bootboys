...
export default class Dock extends React.Component {
            ...
            <div className={classes.master}>
                <div className={classes.sidebyside}>
                    <div className={classes.details}>
                        <h1 className={classes.title}>{this.state.dock.name}</h1>
                        <p>Description: <br/>
                        {this.state.dock.description}</p>
                            <p>facilities:</p>
                            <ul>
                                {this.state.facilitiesToDisplay}
                            </ul>
                        <p>Lenght of the dock: {this.state.dock.length} Meters <br/>
                            Width of the dock: {this.state.dock.width} Meters</p>

                    </div>
                    <div className={classes.rent}>
                        <h2>&euro;{this.state.dock.price}/Night</h2>
                        <Calendar selectRange={true} maxDetail="month" minDetail="year" onChange={this.calculateNights.bind(this)}
                        minDate={new Date()} tileDisabled={({date}) => this.state.rentedAt.includes(date.getTime())} value={this.state.selectedDateRange}/>
                        <div className={classes.sidebyside}>
                            <span>&euro;{this.state.dock.price} * {this.state.nights} nights</span>
                            <span className={classes.result}>&euro;{this.calculateConsumerPrice()}</span>
                        </div>
                        <br/>
                        <div className={classes.sidebyside}>
                            <span>ServiceFee of {this.state.serviceFee}%</span>
                            <span className={classes.result}>&euro;{this.calculateServiceFee()}</span>
                        </div>
                        <div className={classes.total}>
                            <span>Total</span>
                            <span className={classes.result}>&euro;{this.calculateTotal()}</span>
                        </div>
                        <Button onClick={() => this.reserveDock()}>Rent dock</Button>
                    </div>
                </div>
                <div className={classes.location}>
                    <div className={classes.locationDetails}>
                        <p>Country: {this.state.address.country} <br/>
                        City: {this.state.address.city}<br/>
                        Street: {this.state.address.street}<br/>
                        Housenumber: {this.state.address.housenumber}<br/>
                        postalcode: {this.state.address.postalcode}<br/>
                        {this.state.address.state}</p>
                    </div>
    ...
    componentDidMount() {
        ApiService.get('/getDockById/'+this.props.location.state.id).then(handleResponse).then(data => {
            this.SingleDockMap.current.updateMap(data[0]['latitude'], data[0]['longitude'])
            this.buildFacilitiesList(data[0]["facilities"])
    ...
    calculateNights(days){
        if(!this.checkAvailableDays(days)){
            ToastMaker.errorToast('Please select a period that doesnt have days that are already rented out')
            this.setState({
                nights: 0,
                selectedDateRange: null
            })
        ...
        let nights = Math.round(timeBetweenDaysInMili / this.MILISECONDS_IN_DAY - 1)
        this.setState({
            nights: nights
        })
    }
    ...
    registerSocketListener(){
        ...
        socket.on('dockHasBeenRented', (data) => {
            ...
            if (jsonFormat["dockid"] !== this.state.dock.dockid){return;}
            let days = this.createArrayOfDays(jsonFormat["arrivaldate"], jsonFormat["departuredate"])
            this.setState(
                { rentedAt: this.state.rentedAt.concat(days)}
            )
            ToastMaker.warnToast("Somebody just rented this dock from " + jsonFormat["arrivaldate"] + " till " + jsonFormat["departuredate"] )
        })
    }
    ...
    createArrayOfDays(arrivaldate, departuredate){
        let days = [new Date(arrivaldate).getTime(), new Date(departuredate).getTime()]
        let numberOfDays = this.calculateDaysInBetweenArrivalAndDeparture(days[0], days[1])
        let i = 1
    ...
    disableAllRentedDays(dates){
        let disabledDays = []
        for(let i in dates){
            disabledDays = disabledDays.concat(this.createArrayOfDays(dates[i][0],dates[i][1]))
        }
        this.setState(
            { rentedAt: this.state.rentedAt.concat(disabledDays)}
        )

    }

    buildFacilitiesList(facilities){
       let facilitiesObject = JSON.parse(facilities)
        let facilitieListElement = []
        for (let i in facilitiesObject){
            let value = facilitiesObject[i]["description"]
